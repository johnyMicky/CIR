...
import { 
  ...
  Activity, Globe, Zap, ShieldAlert, Terminal, ChevronDown 
} from "lucide-react";
...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-[#f1f5f9] font-sans selection:bg-blue-500/30 overflow-x-hidden relative">
      
      {/* 1. CSS STYLES INTEGRATION (შენი HTML-იდან) */}
      <style>{`
        .glass-card {
      ...
      <div className="blob bottom-[-10%] right-[-10%] animate-pulse"></div>

      {/* SIDEBAR */}
      <aside className="fixed left-0 top-0 hidden h-full w-64 border-r border-white/10 bg-[#020617]/60 backdrop-blur-3xl lg:block z-50">
        ...
        <nav className="p-6 space-y-3">
          <button onClick={() => navigate('/dashboard')} className="w-full flex items-center gap-3 px-5 py-4 bg-blue-600/20 text-blue-400 rounded-2xl font-bold border border-blue-500/20 transition-all"><LayoutDashboard size={20} /> Dashboard</button>
          <button onClick={() => navigate('/history')} className="w-full flex items-center gap-3 px-5 py-4 text-slate-500 hover:text-white transition-all"><History size={20} /> History</button>
          <button className="w-full flex items-center gap-3 px-5 py-4 text-slate-500 hover:text-white transition-all"><Settings size={20} /> Settings</button>
        </nav>
        <div className="absolute bottom-8 left-0 w-full px-6">
          <button onClick={logout} className="flex w-full items-center gap-3 px-5 py-4 text-rose-500/70 hover:text-rose-500 font-bold transition-all rounded-xl hover:bg-rose-500/5"><Lock size={20} /> Logout</button>
        </div>
      </aside>
      ...
      <main className="relative z-10 lg:ml-64 p-4 md:p-10">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
          <div>
            ...
            <p className="text-slate-400 mt-2 font-medium flex items-center gap-2">
                ...
                Session: <span className="text-emerald-500 font-bold">Encrypted & Active</span>
            </p>
          </div>
          <div className="flex items-center gap-4 bg-white/5 backdrop-blur-2xl p-3 rounded-[24px] border border-white/10 shadow-2xl">
            <div className="h-12 w-12 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-700 flex items-center justify-center font-black text-white text-xl shadow-lg">{userData?.firstName?.[0] || 'U'}</div>
            <div className="pr-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
          <div className="lg:col-span-2 glass-card rounded-[40px] p-10 md:p-12 transition-transform duration-500 hover:scale-[1.01]">
              <span className="text-[11px] font-black uppercase tracking-[0.3em] text-slate-400">Node Portfolio Value</span>
              ...
              {Object.entries(market).map(([coin, data]: any) => (
                <div key={coin} className="flex items-center justify-between group">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-[9px] font-black text-emerald-500">Live</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* ASSET TABLE (შენი ცხრილის დიზაინი) */}
        <div className="glass-card rounded-[40px] overflow-hidden shadow-2xl">
          <div className="px-10 py-8 border-b border-white/10 bg-white/[0.02] flex justify-between items-center">
            ...
            <div className="bg-blue-500/10 px-4 py-1.5 rounded-full border border-blue-500/20 text-[10px] font-black text-blue-400 tracking-widest uppercase italic">Integrity: 100%</div>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left">
                <thead><tr className="text-[10px] uppercase tracking-[0.3em] text-slate-600 border-b border-white/5"><th className="px-10 py-6">Asset Source</th><th className="px-10 py-6">Node Balance</th><th className="px-10 py-6">USD Value</th><th className="px-10 py-6">Action</th></tr></thead>
                <tbody className="divide-y divide-white/10">
                {(['ETH', 'BTC', 'USDT'] as CoinKey[]).map((id) => (
                    <tr key={id} className="hover:bg-white/[0.03] transition-all group">
                    <td className="px-10 py-9"><div className="flex items-center gap-5"><img src={market[id].image} className="h-10 w-10" alt="" /><div><div className="font-black text-white text-base tracking-tighter uppercase">{id} Core</div><div className="text-[10px] text-slate-600 font-bold uppercase tracking-widest italic">Mainnet V4.2</div></div></div></td>
                    <td className="px-10 py-9 font-black text-white text-lg">{balances[id.toLowerCase() as keyof typeof balances]} <span className="text-slate-600 text-xs font-bold uppercase">{id}</span></td>
                    <td className="px-10 py-9 font-black text-blue-500 text-lg italic">${(balances[id.toLowerCase() as keyof typeof balances] * market[id].price).toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
      ...
      {receiveOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#020617]/95 backdrop-blur-2xl overflow-y-auto">
          <div className="glass-card w-full max-w-[520px] rounded-[32px] p-8 md:p-10 relative">
            <button onClick={() => setReceiveOpen(false)} className="absolute top-8 right-8 text-slate-500 hover:text-white"><X /></button>
            <h2 className="text-3xl font-black text-white italic uppercase tracking-tighter mb-10 text-center">Deposit Assets</h2>
            <div className="flex gap-2 mb-10">
              {(['ETH', 'BTC', 'USDT'] as CoinKey[]).map(c => (
                <button key={c} onClick={() => setActiveCoin(c)} className={`coin-btn flex-1 py-4 rounded-2xl border transition-all font-black text-[10px] tracking-widest uppercase ${activeCoin === c ? 'active' : 'text-slate-600'}`}>{c}</button>
              ))}
            </div>
            <div className="bg-black/60 p-10 rounded-[40px] border border-white/5 text-center shadow-inner">
              <img src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${userData?.[`${activeCoin.toLowerCase()}_address`] || 'none'}`} className="mx-auto h-48 w-48 rounded-[30px] mb-10 border-[12px] border-white shadow-2xl" alt="QR" />
              <div className="text-[10px] uppercase tracking-[0.3em] text-slate-600 mb-4 font-black italic">Private Wallet Address</div>
      ...
      {/* MODAL: WITHDRAW (Premium Integrated) */}
      {withdrawOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#020617]/95 backdrop-blur-2xl overflow-y-auto">
          {withdrawStep === 0 && (
            <div className="glass-card w-full max-w-[520px] rounded-[32px] p-8 md:p-10 relative">
                <button onClick={() => setWithdrawOpen(false)} className="absolute top-8 right-8 text-slate-500 hover:text-white"><X /></button>
                <h2 className="text-3xl font-black text-white italic uppercase mb-10 text-center">Withdraw Assets</h2>
                <div className="flex gap-2 mb-10">
                  {(['ETH', 'BTC', 'USDT'] as CoinKey[]).map(c => (
                    <button key={c} onClick={() => setActiveCoin(c)} className={`coin-btn flex-1 py-4 rounded-2xl border transition-all font-black text-[10px] tracking-widest uppercase ${activeCoin === c ? 'active' : 'text-slate-600'}`}>{c}</button>
                  ))}
                ...
                <div className="space-y-6">
                    <div className="space-y-3">
                        ...
                        <input type="text" placeholder="0x... wallet node" className="w-full bg-black/40 border border-white/10 rounded-2xl px-6 py-4 text-sm font-bold text-white focus:border-blue-500 outline-none transition-all" onChange={(e) => setWithdrawForm({...withdrawForm, address: e.target.value})} />
                    </div>
                    <div className="space-y-3">
                        <label className="text-[11px] font-black uppercase tracking-widest text-slate-500">Amount to Transfer</label>
                    ...
                    <button onClick={handleWithdrawAction} className="w-full bg-blue-600 py-5 rounded-3xl font-black uppercase text-xs tracking-widest shadow-xl shadow-blue-600/30 hover:translate-y-[-2px] active:scale-95 transition-all mt-6">Confirm Withdrawal</button>
                </div>
            </div>
          )}

          {withdrawStep === 1 && (
            <div className="glass-card w-full max-w-[420px] rounded-[32px] p-12 text-center animate-pulse">
                <RefreshCw size={50} className="animate-spin text-blue-500 mx-auto mb-10" />
                <h2 className="text-xl font-black text-white italic uppercase mb-4 tracking-widest">Routing Assets</h2>
                <p className="text-slate-500 text-xs font-bold uppercase tracking-widest">Node Verification in progress...</p>
            </div>
          )}

          {withdrawStep === 2 && (
             <div className="glass-card w-full max-w-[420px] rounded-[32px] p-12 text-center animate-in zoom-in">
                <div className="w-20 h-20 bg-emerald-500/20 border-2 border-emerald-500 rounded-full flex items-center justify-center mx-auto mb-8 text-emerald-500 shadow-[0_0_30px_rgba(59,130,246,0.4)]">
                  <CheckCircle2 size={40} />
                </div>
                <h3 className="text-2xl font-black text-white italic uppercase mb-2">Request Processed</h3>
                <p className="text-slate-400 text-sm mb-10 italic">Withdrawal is pending admin finalization.</p>
                <button onClick={() => setWithdrawOpen(false)} className="w-full bg-white/10 py-4 rounded-2xl font-black text-xs uppercase hover:bg-white/20 transition-all border border-white/10">Back to Dashboard</button>
             </div>
          )}
        </div>
      )}
